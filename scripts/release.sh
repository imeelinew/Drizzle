...
github_repo="${DRIZZLE_GITHUB_REPOSITORY:-imeelinew/Drizzle}"
team_id="${DRIZZLE_TEAM_ID:-5Q5QT76MJU}"
work_dir=$(mktemp -d /tmp/drizzle-release.XXXXXX)
...
git add Drizzle.xcodeproj/project.pbxproj project.yml
if ! git diff --cached --quiet; then
    git commit -m "Release $tag"
fi

archive_path="$work_dir/Drizzle.xcarchive"
...
app_path="$archive_path/Products/Applications/Drizzle.app"
[[ -d "$app_path" ]] || {
...
archive_file="$work_dir/Drizzle-${version}.zip"
